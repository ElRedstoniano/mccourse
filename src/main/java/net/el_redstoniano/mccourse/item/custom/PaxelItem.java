...
public class PaxelItem extends Item {
    protected static final Map<Block, BlockState> FLATTENABLES = ShovelItemAccessor.getFlattenables();
            /*Maps.<Block, BlockState>newHashMap(
    ...
    public PaxelItem(ToolMaterial toolMaterial, float attackDamageBaseline,
        ...
        super(properties.tool(toolMaterial, ModTags.Blocks.PAXEL_MINEABLE, attackDamageBaseline,
                attackSpeedBaseline, 1f /* for disabling shields*/));
    }

    // Mixed shovel and axe funtionalities
    @Override
    public InteractionResult useOn(UseOnContext pContext) {
        ...
        if (playerHasShieldUseIntent(pContext)) {
            ...
            if (optional.isEmpty()) { // Shovel functionality
                ...
                if (pContext.getClickedFace() == Direction.DOWN) {
                    ...
                    if (blockstate1 != null && level.getBlockState(blockpos.above()).isAir()) { // Flattenable & block above is air
